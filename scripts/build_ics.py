...
def render_index(base_url: str) -> str:
    ...
    lines = [
        ...
        "    .page-track.is-animating { transition-duration: 1120ms; }",
        ...
        "    .intro {",
        "      max-width: 41rem;",
        "      padding: clamp(1.5rem, 4vw, 3.5rem) clamp(0.4rem, 1.2vw, 1rem) clamp(1.1rem, 2vw, 1.8rem);",
        "    }",
        "    .kicker {",
      ...
      '      <section class="page-panel page-panel-subscriptions" id="subscriptions">',
        ...
        *cards,
        ...
        "      createDepthMist() {",
        "        const geometry = new THREE.BufferGeometry();",
        "        const count = window.innerWidth < 768 ? 220 : 360;",
        "        const positions = new Float32Array(count * 3);",
        "        const sizes = new Float32Array(count);",
        "        const seeds = new Float32Array(count);",
        "",
        ...
        "          positions[stride + 2] = -10 - Math.random() * 26;",
        "          sizes[index] = 4 + Math.random() * 10;",
        "          seeds[index] = Math.random() * 100;",
        "        }",
        "",
        "        geometry.setAttribute('position', new THREE.BufferAttribute(positions, 3));",
        ...
        "            void main() {",
        "              vec3 transformed = position;",
        "              transformed.x += sin(time * 0.08 + seed) * 0.9;",
        "              transformed.y += cos(time * 0.11 + seed * 1.3) * 0.7;",
        "              vec4 mvPosition = modelViewMatrix * vec4(transformed, 1.0);",
        "              vAlpha = 0.08 + 0.12 * (0.5 + 0.5 * sin(time * 0.22 + seed));",
        ...
        "              gl_FragColor = vec4(0.88, 0.92, 0.9, strength * vAlpha);",
        "            }`,",
        "        });",
        "",
        "        this.depthMist = new THREE.Points(geometry, material);",
        "        this.depthMist.position.z = -2.5;",
        "        this.scene.add(this.depthMist);",
        "      }",
        "",
        "      createConstellation() {",
        "        const geometry = new THREE.BufferGeometry();",
        ...
        "        if (this.pageTransitionTimer) window.clearTimeout(this.pageTransitionTimer);",
        ...
        "          const disabled = isPrev ? this.pageIndex === 0 : this.pageIndex === this.panels.length - 1;",
        ...
        "        }",
        "        if (this.depthMist?.material?.uniforms?.time) {",
        "          this.depthMist.material.uniforms.time.value = elapsed;",
        "        }",
        "        this.scene.rotation.y = this.pointer.x * 0.12;",
        "        this.scene.rotation.x = this.pointer.y * 0.05;",
        ...
        "        if (this.depthMist) {",
        "          this.depthMist.rotation.z = elapsed * 0.01 * motion;",
        "          this.depthMist.position.x = this.pointer.x * 0.9;",
        "          this.depthMist.position.y = this.pointer.y * 0.55;",
        "        }",
